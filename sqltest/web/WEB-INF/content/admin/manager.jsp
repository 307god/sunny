...
<form action="update-manager" method="get">
	<table id="dataTable" class="display" cellspacing="0" width="100%">
		...
		<%
			List<Administrator> managerList = (List<Administrator>)request.getAttribute("managerList");
			for(Administrator manager : managerList)
			{
		%>
		<tr>
			<td><%=manager.getA_id()%></td>
			<td><%=manager.getA_name()%></td>
			<td><%=manager.getA_pwd()%></td>
			<td><input  type="checkbox" name="cb_delete" value="<%=manager.getA_id()%>"/></td>
		</tr>
		<%
			}
		%>
